...
ASTNode *casting_to_ast(CSTNode *cst_node)
{
    ...
    for (int i = 0; i < cst_node->child_count; i++)
    {
        ...
        if (!strcmp(child->symbol, "as"))
        {
            ...
            if (i > 0)
                expr = expr_to_ast(cst_node->children[i - 1]);
...
ASTNode *getter_to_ast(CSTNode *cst_node)
{
    ...
    for (int i = 0; i < cst_node->child_count; i++)
    {
        ...
        if (!strcmp(child->symbol, "."))
        {
            ...
            if (i > 0)
                instance = expr_to_ast(cst_node->children[i - 1]);
            if (i + 1 < cst_node->child_count)
            {
...
ASTNode **process_var_binding_list(CSTNode *var_binding_list, int *count)
{
    *count = 0;
    ...
    for (int i = 0; i < var_binding_list->child_count; i++)
    {
        ...
        else if (!strcmp(child->symbol, "VarBindingListTail"))
        {
            // Procesar recursivamente la cola
            int tail_count = 0;
            ASTNode **tail_decls = process_var_binding_list_tail(child, &tail_count);
            if (tail_decls && tail_count > 0)
            {
                declarations = realloc(declarations, (*count + tail_count) * sizeof(ASTNode *));
                for (int j = 0; j < tail_count; j++)
                {
                    declarations[*count + j] = tail_decls[j];
                }
                *count += tail_count;
                free(tail_decls);
            }
...
ASTNode *expr_to_ast(CSTNode *cst_node)
{
        ...
        !strcmp(cst_node->symbol, "Unary"))
    {
        ...
        {
            CSTNode *child = cst_node->children[i];
            if (!child || strcmp(child->symbol, "ε") == 0)
                continue;
...
ASTNode *primary_to_ast(CSTNode *cst_node)
{
    ...
    for (int i = 0; i < cst_node->child_count; i++)
    {
        ...
        else if (!strcmp(child->symbol, "LPAREN") || !strcmp(child->symbol, "("))
        {
            // Busca el hijo Expr entre los hijos
            for (int j = 0; j < cst_node->child_count; j++)
            {
                CSTNode *expr_child = cst_node->children[j];
                if (expr_child && !strcmp(expr_child->symbol, "Expr"))
                {
                    base = expr_to_ast(expr_child);
                    DEBUG("Base (Expr) procesada\n");
                    break;
                }
            }
        }
...
ASTNode *process_primed_expression(CSTNode *primed_node, ASTNode *left_operand)
{
    ...
    if (op_node && right_node)
    {
        ...
        if (!strcmp(op_str, "+"))
            ...
            op = OP_MOD;
        else if (!strcmp(op_str, ">") || !strcmp(op_str, "GREATER"))
            op = OP_GR;
        else if (!strcmp(op_str, "<") || !strcmp(op_str, "LESS"))
            op = OP_LS;
        else if (!strcmp(op_str, "==") || !strcmp(op_str, "EQUALSEQUALS"))
            op = OP_EQ;
        else if (!strcmp(op_str, "!=") || !strcmp(op_str, "NEQUALS"))
            op = OP_NEQ;
        else if (!strcmp(op_str, ">=") || !strcmp(op_str, "EGREATER"))
            op = OP_GRE;
        else if (!strcmp(op_str, "<=") || !strcmp(op_str, "ELESS"))
            op = OP_LSE;
        else if (!strcmp(op_str, "&"))
            op = OP_AND;
        else if (!strcmp(op_str, "|"))
