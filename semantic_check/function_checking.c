...
void check_function_call(Visitor* v, ASTNode* node, Type* type) {
    ...
    if (funcData->func) {
        ...
        node->dynamic_type = funcData->func->result_type;
    }
...
void check_function_dec(Visitor* v, ASTNode* node, Type* type) {
    ...
    fprintf(stderr, "ESTOY EN EL CHECK DEC FUNCTION\n");
    ...
    for (int i = 0; i < node->data.func_node.arg_count; i++)
    {
        ...
        if (strcmp(params[i]->static_type, "") && !param_type) {
            ...
            if (item) {
                ...
                params[i]->dynamic_type = &TYPE_ERROR;
            }
        ...
        if (!strcmp(params[i]->static_type, "")) {
            ...
            params[i]->dynamic_type = &TYPE_ANY;
        } else if (param_type) {
            params[i]->return_type = param_type->type;
            params[i]->dynamic_type = param_type->type;
        }

        // Declare parameters in the function scope
        declare_symbol(
            node->scope, params[i]->data.variable_name,
            params[i]->return_type, 1, NULL, params[i]->dynamic_type
        );
    ...
    if (!func) {
        ...
        for (int i = 0; i < node->data.func_node.arg_count; i++)
        {
            ...
            node->data.func_node.args[i]->dynamic_type = param_types[i];
        }
        ...
        body->dynamic_type = inferried_type;
    ...
    fprintf(stderr, "asasdasd :%s y  %s\n", node->data.func_node.name, node->return_type->name);
