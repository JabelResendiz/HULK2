...
LLVMValueRef generate_string_concatenation(LLVMValueRef L, LLVMValueRef R, int is_double_concat) {
    ...
    // Ensure strings are properly loaded if they are pointers
    if (LLVMGetTypeKind(LLVMTypeOf(L)) == LLVMPointerTypeKind &&
        LLVMGetTypeKind(LLVMGetElementType(LLVMTypeOf(L))) == LLVMPointerTypeKind) {
        L = LLVMBuildLoad2(builder, LLVMPointerType(LLVMInt8Type(), 0), L, "L_loaded");
    }
    if (LLVMGetTypeKind(LLVMTypeOf(R)) == LLVMPointerTypeKind &&
        LLVMGetTypeKind(LLVMGetElementType(LLVMTypeOf(R))) == LLVMPointerTypeKind) {
        R = LLVMBuildLoad2(builder, LLVMPointerType(LLVMInt8Type(), 0), R, "R_loaded");
    }
