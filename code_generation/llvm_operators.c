...
LLVMValueRef handle_object_operation(LLVMValueRef left, LLVMValueRef right, int op) {
    ...
    if (LLVMGetTypeKind(LLVMTypeOf(left)) == LLVMPointerTypeKind) {
        left = LLVMBuildLoad2(builder, LLVMGetElementType(LLVMTypeOf(left)), left, "left_loaded");
    }
    if (LLVMGetTypeKind(LLVMTypeOf(right)) == LLVMPointerTypeKind) {
        right = LLVMBuildLoad2(builder, LLVMGetElementType(LLVMTypeOf(right)), right, "right_loaded");
    }
...
LLVMValueRef generate_binary_operation(LLVM_Visitor* v, ASTNode* node) {
    ...
    // If values are pointers to their types, load them before operation
    if (LLVMGetTypeKind(LLVMTypeOf(L)) == LLVMPointerTypeKind &&
        !type_equals(node->data.op_node.left->return_type, &TYPE_STRING)) {
        L = LLVMBuildLoad2(builder, LLVMGetElementType(LLVMTypeOf(L)), L, "left_loaded");
    }
    if (LLVMGetTypeKind(LLVMTypeOf(R)) == LLVMPointerTypeKind &&
        !type_equals(node->data.op_node.right->return_type, &TYPE_STRING)) {
        R = LLVMBuildLoad2(builder, LLVMGetElementType(LLVMTypeOf(R)), R, "right_loaded");
    }
    ...
    if (type_equals(node->data.op_node.left->return_type, &TYPE_NUMBER)) {
        // Load values from allocas if necessary
        if (LLVMGetTypeKind(LLVMTypeOf(L)) == LLVMPointerTypeKind &&
            LLVMGetTypeKind(LLVMGetElementType(LLVMTypeOf(L))) == LLVMDoubleTypeKind) {
            L = LLVMBuildLoad2(builder, LLVMDoubleType(), L, "loaded_left");
        }
        if (LLVMGetTypeKind(LLVMTypeOf(R)) == LLVMPointerTypeKind &&
            LLVMGetTypeKind(LLVMGetElementType(LLVMTypeOf(R))) == LLVMDoubleTypeKind) {
            R = LLVMBuildLoad2(builder, LLVMDoubleType(), R, "loaded_right");
        }
    ...
    if (type_equals(node->data.op_node.left->return_type, &TYPE_BOOLEAN)) {
        // Load boolean values from allocas if necessary
        if (LLVMGetTypeKind(LLVMTypeOf(L)) == LLVMPointerTypeKind &&
            LLVMGetTypeKind(LLVMGetElementType(LLVMTypeOf(L))) == LLVMIntegerTypeKind) {
            L = LLVMBuildLoad2(builder, LLVMInt1Type(), L, "loaded_left_bool");
        }
        if (LLVMGetTypeKind(LLVMTypeOf(R)) == LLVMPointerTypeKind &&
            LLVMGetTypeKind(LLVMGetElementType(LLVMTypeOf(R))) == LLVMIntegerTypeKind) {
            R = LLVMBuildLoad2(builder, LLVMInt1Type(), R, "loaded_right_bool");
        }
...
LLVMValueRef generate_unary_operation(LLVM_Visitor* v, ASTNode* node) {
    ...
    switch (node->data.op_node.op) {
        ...
        case OP_NOT:
            if (type_equals(node->data.op_node.left->return_type, &TYPE_BOOLEAN)) {
                // Load boolean value from alloca if necessary
                if (LLVMGetTypeKind(LLVMTypeOf(operand)) == LLVMPointerTypeKind &&
                    LLVMGetTypeKind(LLVMGetElementType(LLVMTypeOf(operand))) == LLVMIntegerTypeKind) {
                    operand = LLVMBuildLoad2(builder, LLVMInt1Type(), operand, "loaded_bool");
                }
