...
LLVMValueRef print_function(LLVM_Visitor* v, ASTNode* node) {
    ...
    // Detectar si el valor es numérico basado en su tipo LLVM
    LLVMTypeRef arg_type = LLVMTypeOf(arg);
    int is_numeric = LLVMGetTypeKind(arg_type) == LLVMDoubleTypeKind;

    // Seleccionar formato según el tipo del argumento
    if (is_numeric || type_equals(arg_node->return_type, &TYPE_NUMBER)) {
        format = "%g\n";
...
LLVMValueRef basic_functions(
    ...
    // Load the value from alloca if necessary
    if (LLVMGetTypeKind(LLVMTypeOf(arg)) == LLVMPointerTypeKind &&
        LLVMGetTypeKind(LLVMGetElementType(LLVMTypeOf(arg))) == LLVMDoubleTypeKind) {
        arg = LLVMBuildLoad2(builder, LLVMDoubleType(), arg, "loaded_arg");
    }
